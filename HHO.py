...
def HHO(objf,lb,ub,dim,SearchAgents_no,Max_iter,trainInput,trainOutput):
    ...
    while t<Max_iter:
                ...
                if r<0.5 and abs(Escaping_Energy)>=0.5: # Soft besiege Eq. (10) in paper
                    ...
                    for j in range(0,dim):
                     ...
                     if (random.random()<ss): 
                             ...
                             X1[j]=0;
                    if objf(X1,trainInput,trainOutput,dim)< fitness: # improved move?
                        X[i,:] = X1.copy()
        ...
        if (t%1==0):
               print(['At iteration '+ str(t)+ ' the best fitness is '+ str(Rabbit_Energy)+', the best number of features: '+str(featurecount)])
